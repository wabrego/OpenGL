...
//---------------------------------------------------------------------------------------
Shaders::Shaders(const char *vShaderFile, const char *fShaderFile) { 
...
void Shaders::setTextures(const std::string &name, Textures value) {
    ...
    glActiveTexture(GL_TEXTURE0 + value.diffuse->texture);
    glBindTexture(GL_TEXTURE_2D,value.diffuse->texture);
    glUniform1i(glGetUniformLocation(program,(name+".diffuse").c_str()), value.diffuse->texture);

    glActiveTexture(GL_TEXTURE0 + value.specular->texture);
    glBindTexture(GL_TEXTURE_2D,value.specular->texture);
    glUniform1i(glGetUniformLocation(program,(name+".specular").c_str()), value.specular->texture);

    glActiveTexture(GL_TEXTURE0 + value.emissive->texture);
    glBindTexture(GL_TEXTURE_2D,value.emissive->texture);
    glUniform1i(glGetUniformLocation(program,(name+".emissive").c_str()), value.emissive->texture);
    
    if(value.normal!=NULL) { 
        glUniform1i(glGetUniformLocation(program,"unormals"),1);
        glActiveTexture(GL_TEXTURE0 + value.normal->texture);
        glBindTexture(GL_TEXTURE_2D,value.normal->texture);
        glUniform1i(glGetUniformLocation(program,(name+".normal"  ).c_str()), value.normal->texture  );
